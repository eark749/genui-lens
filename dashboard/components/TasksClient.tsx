...
function rateColor(r: number) {
  if (r >= 0.7) return "text-green-600 bg-green-50";
  if (r >= 0.4) return "text-yellow-700 bg-yellow-50";
    ...
    <div>
      ...
      <div className="mb-4">
        <input
          ...
          className="w-full max-w-sm px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      </div>
      ...
      <div className="space-y-2">
        ...
        {clean.map((row) => (
          <div
            key={row.intent}
            className="bg-white rounded-lg border border-gray-200 px-4 py-3 hover:bg-gray-50 transition-colors flex items-center justify-between gap-4"
          >
            ...
            <span
              className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold flex-shrink-0 ${rateColor(row.success_rate)}`}
            >
